...
{
    public class CppSources : CppCodeGenerator
    {
        ...
        public string AssemblyId => CppGenerator.AssemblyId(Unit);

        Options EmbedOptions => Options as Options;

        public override void WriteHeaders()
        {
            WriteLine($"#include \"{Unit.FileNameWithoutExtension}.hpp\"");
        ...
        public void GenerateMethodLookup(Method method)
        {
            ...
            WriteLine($"{methodId} = mono_embeddinator_lookup_method({methodNameId}, {@class.Name}::class_{@class.Name});");
            ...
            string @object, string field)
        {
            ...
            default:
                ...
                return $"{objExpr}{CppGenerator.ObjectInstanceId}->{field}";
            }
        }
        ...
        public virtual void GenerateMethodInitialization(Method method)
        {
            ...
            if (method.IsConstructor)
            {
                ...
                if (Options.GeneratorKind == GeneratorKind.C)
                    ...
                    WriteLine("this->{0} = ({1}*) mono_embeddinator_create_object({2});",
                        CppGenerator.ObjectInstanceId, GenerateObjectTypesPass.MonoEmbedObject.Name,
                        instanceId);
            }
            else if (!method.IsStatic)
            {
                var handle = "this->__object->_handle";
