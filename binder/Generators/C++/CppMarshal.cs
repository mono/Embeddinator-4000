...
{
    public class CppMarshalPrinter : MarshalPrinter<MarshalContext>
    {
        public CppMarshalPrinter(MarshalContext marshalContext)
            : base(marshalContext)
        {
        }

        public CppManagedToNativeTypePrinter CppTypePrinter =>
            CppGenerator.GetCppTypePrinter(CppSharp.Generators.GeneratorKind.CPlusPlus);
    ...
    public class CppMarshalManagedToNative : CppMarshalPrinter
    {
        public Options Options { get; private set; }
        public bool UnboxPrimitiveValues { get; set; }

        public CppMarshalManagedToNative(Options options, MarshalContext marshalContext)
            : base(marshalContext)
        {
            Options = options;
            ...
            TypeQualifiers quals)
        {
            ...
            var typePrinter = CppTypePrinter;
            ...
            support.WriteLine("MonoClass* {0} = mono_class_get_element_class({1});",
                elementClassId,
                CppMarshalNativeToManaged.GenerateArrayTypeLookup(array.Array.Type, support));
            ...
            var isValueType = CppMarshalNativeToManaged.IsValueType(array.Array.Type);
            ...
            var marshal = new CppMarshalManagedToNative(Options, ctx) { UnboxPrimitiveValues = false };
        ...
        {
            var typeName = @class.Visit(CppTypePrinter);
            var objectId = $"{Context.ArgName}_obj";
            Context.SupportBefore.WriteLine($"{typeName}* {objectId} = {Context.ArgName} ? ({typeName}*) mono_embeddinator_create_object({Context.ArgName}) : 0;");
    ...
    public class CppMarshalNativeToManaged : CppMarshalPrinter
    {
        public Options Options { get; private set; }
        public bool PrimitiveValuesByValue { get; set; }

        public CppMarshalNativeToManaged (Options options, MarshalContext marshalContext)
            : base (marshalContext)
        {
            Options = options;
            ...
            TypeQualifiers quals)
        {
            ...
            var marshal = new CppMarshalNativeToManaged (Options, ctx) { PrimitiveValuesByValue = true };
